...
- **abdddd5** validate external source→binary: `extract_source_apis` (sce* scanning without regex) + `compute_source_correlation` (finds release sample projects, compares source APIs vs binary imports, reports scanned/matched/mismatched, PASS/INSUFFICIENT), .exe differential validation via explicit --tool remains SKIPPED, validated on single sample convreverb (1 project, 20 APIs, 30 imports, 0 matched) and Tools 3198 files (82 .exe), full samples root SKIPPED handling — no proprietary copy, no hardcode, external roots as configurable oracles.
- **8ba6fbe** validate perf: `compute_source_correlation` now filters `file_list` for release sample projects and source/binary files (no re-walk), handles full 27-game 76154 files + full samples root without timeout, validated on full ROMS inventory (76154 files, 30 exts, 530 executable, 514 shader) and Tools 3198, single sample still PASS.
...
- SDK Timeline tab `html.rs:866-922`: a **hardcoded static table** (platform SDK/UE/EMC), not derived from `SdkDatabase`/`ps5-abi`.
