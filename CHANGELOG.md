...
- **`libdbg_basic` fixture**: a self-authored `ET_SCE_DYNEXEC` mirroring the SDK `libdbg` sample's flow — imports `sceDbgSetMinimumLogLevel`, `puts`, `rand`, `sceDbgLoggingHandler`, emits a banner plus three log lines with deterministic random payloads, and produces a 16-call ordered import trace. Regenerated byte-exact into `data/test/generated_elfs/`
