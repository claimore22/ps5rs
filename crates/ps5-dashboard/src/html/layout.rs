...
fn render_game_detail_html(game: &crate::data::GameDetail, data: &DashboardData) -> String {
    ...
    out.push_str(&format!(
        r#"<div class="k">Entry Point</div><div class="v" style="font-family:monospace">{}</div>"#,
        game.entry_point
    ));
    ...
    out.push_str(&format!(
        r#"<div class="k">ELF Type</div><div class="v">0x{:x}</div>"#,
        game.elf_type
    ));
    ...
    if let Some(bs) = &game.build_system {
        out.push_str(&format!(
            r#"<div class="k">Build System</div><div class="v">{}</div>"#,
            bs
        ));
    }
    if let Some(sd) = &game.source_depot {
        out.push_str(&format!(
            r#"<div class="k">Source Depot</div><div class="v">{}</div>"#,
            sd
        ));
    }
    out.push_str("</div>");
    if !game.engine_evidence.is_empty() {
        ...
        for ev in &game.engine_evidence {
            out.push_str(&format!(
                r#"<tr><td style="font-family:monospace;font-size:0.72rem">{}</td></tr>"#,
                ev
            ));
        }
    ...
    if !game.lib_versions.is_empty() {
        ...
        for lv in &game.lib_versions {
            out.push_str(&format!(
                r#"<tr><td style="font-family:monospace">{}</td><td>{}</td></tr>"#,
                lv.name, lv.version_string
            ));
        }
    ...
    for seg in &game.segments {
        out.push_str(&format!(
            ...
            seg.index, seg.seg_type, seg.vaddr, seg.filesz
        ));
    }
    out.push_str("</tbody></table></div></div>");
