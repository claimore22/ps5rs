...
pub fn export_analysis(db: &AnalysisDatabase, writer: &mut dyn Write) -> std::io::Result<()> {
    writeln!(
        writer,
        "game,platform,file_size,imports,relocations,symbols,has_tls"
    )?;
...
pub fn export_nid_frequency(freq: &NidFrequency, writer: &mut dyn Write) -> std::io::Result<()> {
    ...
    for entry in &freq.entries {
        ...
        writeln!(
            writer,
            "{},{},{},{}",
            entry.nid_hash, entry.name, entry.count, game_list,
        )?;
    }
    Ok(())
}
...
pub fn export_library_versions(
    ...
    writeln!(
        writer,
        "library,version_raw,version_string,game_count,games"
    )?;
...
pub fn export_unknown_nids(
    report: &crate::reports::unknown_nids::UnknownNidReport,
    writer: &mut dyn Write,
) -> std::io::Result<()> {
...
mod tests {
    ...
    fn csv_unresolved_header_and_rows() {
        ...
        db.games[0]
            .imports
            .push(crate::model::make_import("zzz", "?", 3, "libC"));
