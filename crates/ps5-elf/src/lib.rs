...
impl<'a> ElfImage<'a> {
    ...
    pub fn parse(data: &'a [u8], phdr_file_offsets: Option<&[u64]>) -> ps5_format::Result<Self> {
        ...
        let import_libs = dynamic::parse_import_libs(&dyn_entries, &data[strtab_offset as usize..]);
        let needed_files = dynamic::parse_needed_files(&dyn_entries, &data[strtab_offset as usize..]);
