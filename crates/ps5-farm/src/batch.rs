...
pub struct GameLoadReport {
    game: String,
    path: String,
    load_report: Option<report::LoadReport>,
    error: Option<String>,
    #[serde(default)]
    total_exports: usize,
    #[serde(default)]
...
pub fn run_batch_load(
    ...
    if games.is_empty() {
        return Err(format!(
            "no games found (no eboot.bin) in: {}",
            games_dir.display()
        ));
    }
    ...
    let offline_fp = fingerprint::offline_fingerprint(offline_dir)
        .map_err(|e| format!("cannot fingerprint offline dir: {e}"))?;
    ...
    if let Err(e) = std::fs::create_dir_all(&games_out) {
        return Err(format!(
            "cannot create games output directory {}: {e}",
            games_out.display()
        ));
    }
...
mod tests {
    ...
    fn stored_report(game: &str, freshness: Option<ReportFreshness>) -> GameLoadReport {
        let load_report: report::LoadReport = serde_json::from_value(serde_json::json!({
            "modules": [],
