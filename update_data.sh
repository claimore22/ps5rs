...
cargo run -p ps5-cli -- middleware "$ROMS" --format json --output "$OUT/reports/middleware.json" || echo "middleware optional"
...
cargo run -p ps5-cli -- dashboard "$OUT" --output "$OUT/dashboard/index.html" --games "$ROMS"
# Multi-page dir variant:
